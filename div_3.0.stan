...
parameters {
  ...
  real zb_abx1[n_abx, n_lag];
  real za_abx1[n_lag];
  real<lower=0> s_abx1[n_lag];
...
transformed parameters {
  ...
  for (i in 1:n_specimens) {
    ...
    for (j in 1:n_abx) {
      for (k in 1:n_lag) {
        b_abx1[j, k] = za_abx1[k] + zb_abx1[j, k] * s_abx1[k];
        b_abx2[subject_id, j, k] = za_abx2[j, k] + zb_abx2[subject_id, j, k] * s_abx2[j, k];
        mu[i] += (b_abx1[j, k] + b_abx2[subject_id, j, k]) * abx[i, j, k];
      }
    }
  }
}
model {
  ...
  for (k in 1:n_lag) {
    za_abx1[k] ~ normal(0, 1);
    s_abx1[k] ~ normal(0, 1);
    for (j in 1:n_abx) {
      zb_abx1[j, k] ~ normal(0, 1);
