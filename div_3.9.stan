  ...
  for (i in 1:n_specimens) {
    ...
    for (j in 1:n_abx) {
      for (k in 1:n_continuous) {
        b_abx2[subject_id, j, k] = za_abx2[j] + zb_abx2[subject_id, j, k] * s_abx2[j];
...
model {
  ...
  for (j in 1:n_abx) {
    za_abx2[j] ~ normal(0, 10);
    s_abx2[j] ~ normal(0, 10);
    for (k in 1:n_continuous) {
...
//   real b_abx_agg[n_subjects, n_abx, n_continuous];
//   for (i in 1:n_subjects) {
//     for (j in 1:n_abx) {
//       for (k in 1:n_continuous) {
//         b_abx_agg[i, j, k] = sum(b_abx2[i, j, 1:k]);
//       }
//     }
//   }
// }
